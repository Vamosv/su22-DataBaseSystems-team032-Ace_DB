...
def update_user_info():
    ...
    sql_3 = "select * from User_info; "
    results = conn.execute(sql_3).fetchall()
    print (results)
    conn.close()
    result_list_info = []
    for result in results:
        ...
        result_list_info.append(item)
    return render_template('update.html',items_update=result_list, info = result_list_info)
...
def sp():
    conn = db.connect()
    sql1 = "call User_upvote_5more_16()"
    results = conn.execute(sql1).fetchall()
    # print(results)
    conn.close()
    result_list_1 = []
    for result in results:
    ...
    conn = db.connect()
    sql2 = "call User_review_16()"
    results = conn.execute(sql2).fetchall()
    # print(results)
    conn.close()
    result_list_2 = []
    for result in results:
    ...
    return render_template('sp1.html', items1=result_list_1, items2=result_list_2)
